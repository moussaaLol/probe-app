...
function loadNotifications() {
    ...
    // Get reference to the "notif" collection
    db.collection('notif')
        ...
        .then(snapshot => {
            ...
            snapshot.forEach(doc => {
                ...
                // Format the notification message and timestamp
                ...
                item.addEventListener('click', () => {
                    if (!notification.read) {
                        ...
                        db.collection('notif').doc(doc.id).update({
                            read: true,
                            readAt: new Date()
                        }).then(() => {
        ...
        .catch(error => {
            ...
            notificationList.innerHTML = '<li class="notification-item">Error loading notifications</li>';
        });
}
...
function setupNotificationListener() {
    ...
    return db.collection('notif')
        .where('userId', '==', auth.currentUser.uid)
        .orderBy('createdAt', 'desc')
        .limit(10)
        .onSnapshot(snapshot => {
            loadNotifications(); // Reload when notifications change
...
// Show a toast message for new notifications
function showNotificationToast(message) {
    ...
    toast.textContent = message || 'New notification';
...
// Update the auth state listener to include notification setup
auth.onAuthStateChanged(user => {
    if (user) {
        ...
        loadNotifications();
        setupNotificationListener(); // Set up real-time listener
    } else {
